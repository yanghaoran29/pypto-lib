...
def qkv_proj_rope(
    ...
    for tg_idx in pl.spmd(t_dim // T_TILE, name_hint="qr_rms_norm_quant"):
        ...
        for qa in pl.pipeline(0, Q_LORA, QUANT_TILE, stage=2):
            ...
            qr_q_half = pl.cast(pl.cast(qr_q_i32, target_type=pl.FP32), target_type=pl.FP16, mode="round")
    ...
    for hg_idx in pl.spmd(H // 2, name_hint="q_head_rms_nope_rope"):
        hg = hg_idx * 2
        # half-split (NeoX) forward RoPE -> no index/sign build; read cos/sin at native half width.
        for tg_idx in pl.range(t_dim // Q_ROPE_T_TILE):
            tg = tg_idx * Q_ROPE_T_TILE
            q_cos_h = pl.cast(rope_cos_view[tg : tg + Q_ROPE_T_TILE, 0 : ROPE_HALF], target_type=pl.FP32)
            q_sin_h = pl.cast(rope_sin_view[tg : tg + Q_ROPE_T_TILE, 0 : ROPE_HALF], target_type=pl.FP32)
            for h_inner in pl.range(2):
                ...
                q_lo = q_proj_fp32[tg : tg + Q_ROPE_T_TILE, h0 + NOPE_DIM : h0 + NOPE_DIM + ROPE_HALF]
                q_hi = q_proj_fp32[tg : tg + Q_ROPE_T_TILE, h0 + NOPE_DIM + ROPE_HALF : h0 + HEAD_DIM]
                q_rope_rot = pl.concat(pl.sub(pl.mul(q_lo, q_cos_h), pl.mul(q_hi, q_sin_h)),
                                       pl.add(pl.mul(q_hi, q_cos_h), pl.mul(q_lo, q_sin_h)))
    ...
    for tg_idx in pl.spmd(t_dim // KV_RMS_T_TILE, name_hint="kv_rms_norm_rope"):
        ...
        # half-split (NeoX) forward RoPE on the normed+gamma'd chunk: pair lane i with i+ROPE_HALF.
        kv_cos_h = pl.cast(rope_cos_view[tg : tg + KV_RMS_T_TILE, 0 : ROPE_HALF], target_type=pl.FP32)
        kv_sin_h = pl.cast(rope_sin_view[tg : tg + KV_RMS_T_TILE, 0 : ROPE_HALF], target_type=pl.FP32)
        kv_lo = kv_rope_norm_chunk[0 : KV_RMS_T_TILE, 0 : ROPE_HALF]
        kv_hi = kv_rope_norm_chunk[0 : KV_RMS_T_TILE, ROPE_HALF : ROPE_DIM]
        kv_rope_rot = pl.concat(pl.sub(pl.mul(kv_lo, kv_cos_h), pl.mul(kv_hi, kv_sin_h)),
                                pl.add(pl.mul(kv_hi, kv_cos_h), pl.mul(kv_lo, kv_sin_h)))
...
def golden_qkv_proj_rope(tensors):
    ...
    def apply_rope(x_rope, cos, sin):
        # half-split (NeoX): pair lane i with lane i+ROPE_HALF (contiguous halves).
        x_lo, x_hi = x_rope[..., :ROPE_HALF], x_rope[..., ROPE_HALF:]
        cos_v = cos[..., :ROPE_HALF]
        sin_v = sin[..., :ROPE_HALF]
