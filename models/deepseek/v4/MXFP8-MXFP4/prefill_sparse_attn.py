...
def prefill_sparse_attn(
    ...
    # Inverse RoPE (half-split / NeoX, gather-free): no cos/sin dup-gather precompute; the
    # rope scope below reads freqs_cos/freqs_sin at native half width and rotates lo/hi halves.
    ...
    with pl.spmd((H // 4) * (T // ROPE_OUT_TOK_TILE), name_hint="rope") as rope_tid:
        ...
        # half-split cos/sin at native half width for this token tile (per-token per-angle).
        r_cos_h = pl.cast(freqs_cos[rp_t0 : rp_t0 + ROPE_OUT_TOK_TILE, 0 : ROPE_HALF], target_type=pl.FP32)
        r_sin_h = pl.cast(freqs_sin[rp_t0 : rp_t0 + ROPE_OUT_TOK_TILE, 0 : ROPE_HALF], target_type=pl.FP32)
        for rp_hl in pl.range(0, 4):
            ...
            rp_o0 = rp_g * T + rp_t0
            # Rotate the whole ROPE_DIM segment at once: pair lane i with lane i+ROPE_HALF.
            r_full = pl.reshape(
                attn_rope_stage_3d[rp_t0 : rp_t0 + ROPE_OUT_TOK_TILE, rp_gh : rp_gh + 1, 0 : ROPE_DIM],
                [ROPE_OUT_TOK_TILE, ROPE_DIM])
            r_lo = r_full[0 : ROPE_OUT_TOK_TILE, 0 : ROPE_HALF]
            r_hi = r_full[0 : ROPE_OUT_TOK_TILE, ROPE_HALF : ROPE_DIM]
            r_out_lo = pl.add(pl.mul(r_lo, r_cos_h), pl.mul(r_hi, r_sin_h))
            r_out_hi = pl.sub(pl.mul(r_hi, r_cos_h), pl.mul(r_lo, r_sin_h))
            r_rot = pl.cast(pl.concat(r_out_lo, r_out_hi), target_type=pl.BF16, mode="rint")
            o_packed[rp_o0 : rp_o0 + ROPE_OUT_TOK_TILE, rp_col : rp_col + ROPE_DIM] = r_rot
...
def golden_prefill_sparse_attn(tensors):
    ...
    # Half-split (NeoX) inverse RoPE: pair lane i with lane i+ROPE_HALF (contiguous halves).
    rope_lo = o[..., NOPE_DIM:NOPE_DIM + ROPE_HALF]
    rope_hi = o[..., NOPE_DIM + ROPE_HALF:]
