...
def indexer(
    ...
    for idx in pl.spmd(T * IDX_N_HEADS // ROPE_ROW_TILE, name_hint="qr_rope"):
        ...
        # half-split (NeoX) forward RoPE: pair lane i with i+HALF; cos_b/sin_b half width [1,HALF].
        qr_lo = qr_proj_flat[o0 : o0 + ROPE_ROW_TILE, IDX_NOPE_HEAD_DIM : IDX_NOPE_HEAD_DIM + ROPE_HEAD_DIM // 2]
        qr_hi = qr_proj_flat[o0 : o0 + ROPE_ROW_TILE, IDX_NOPE_HEAD_DIM + ROPE_HEAD_DIM // 2 : IDX_HEAD_DIM]
        rope_rot = pl.concat(pl.sub(pl.col_expand_mul(qr_lo, cos_b), pl.col_expand_mul(qr_hi, sin_b)),
                             pl.add(pl.col_expand_mul(qr_hi, cos_b), pl.col_expand_mul(qr_lo, sin_b)))
