...
def sparse_attn_swa(
    ...
    # NOTE: half-split (NeoX) inverse RoPE -> no cos/sin dup-gather precompute. merge_norm
    # reads freqs_cos/freqs_sin at native half width (freqs are cat([cos_half,cos_half]), so
    # cols [0:HALF_ROPE] == the per-angle table) and rotates the contiguous lo/hi halves.
    ...
    with pl.spmd(T * (H // H_TILE), name_hint="merge_norm") as merge_tid:
        ...
        # Inverse RoPE (half-split / NeoX, gather-free): pair lane i with lane i+HALF_ROPE
        # (contiguous halves) instead of interleaved (2i, 2i+1). cos/sin at native half width,
        # col_expand'd over the H_TILE head rows. Conjugate (inverse) rotation:
        #   out_lo = lo*cos + hi*sin ;  out_hi = hi*cos - lo*sin
        m_lo = n_full[0 : H_TILE, NOPE_DIM : NOPE_DIM + HALF_ROPE]
        m_hi = n_full[0 : H_TILE, NOPE_DIM + HALF_ROPE : HEAD_DIM]
        m_cos_h = pl.cast(freqs_cos[m_t : m_t + 1, 0 : HALF_ROPE], target_type=pl.FP32)
        m_sin_h = pl.cast(freqs_sin[m_t : m_t + 1, 0 : HALF_ROPE], target_type=pl.FP32)
        m_out_lo = pl.add(pl.col_expand_mul(m_lo, m_cos_h), pl.col_expand_mul(m_hi, m_sin_h))
        m_out_hi = pl.sub(pl.col_expand_mul(m_hi, m_cos_h), pl.col_expand_mul(m_lo, m_sin_h))
        n_rope_bf16 = pl.cast(pl.concat(m_out_lo, m_out_hi), target_type=pl.BF16, mode="rint")
            ...
            for g in pl.range(O_GROUPS):
                ...
                with pl.at(level=pl.Level.CORE_GROUP, name_hint="quant",
                    ...
                    for qt in pl.pipeline(t_base, t_base + QUANT_T_CHUNK, QUANT_TOKEN_TILE, stage=2):
                        ...
                        oq_half = pl.cast(pl.cast(oq_i32, target_type=pl.FP32), target_type=pl.FP16, mode="round")
                        ...
                        if T_PAD > T:
                            ...
                            zero_half = pl.cast(pl.cast(zero_i32, target_type=pl.FP32), target_type=pl.FP16, mode="round")
...
def golden_sparse_attn(tensors):
    ...
    # Half-split (NeoX) inverse RoPE: pair lane i with lane i+HALF_ROPE (contiguous halves),
    # matching the gather-free kernel (m_lo/m_hi + concat). cos/sin at native half width.
    ...
    inv_lo = (rope_lo * cos_half + rope_hi * sin_half).to(torch.bfloat16).float()
    inv_hi = (rope_hi * cos_half - rope_lo * sin_half).to(torch.bfloat16).float()
