...
def prefill_indexer(
    ...
    for idx in pl.parallel(0, IDX_N_HEADS * IDX_HEAD_DIM // Q_OUT_TILE, 1):
        with pl.at(level=pl.Level.CORE_GROUP, name_hint="prefill_idx_qr_proj"):
            o0 = idx * Q_OUT_TILE
    ...
    for idx in pl.parallel(0, T * IDX_N_HEADS // ROPE_ROW_BLOCK, 1):
        with pl.at(level=pl.Level.CORE_GROUP, name_hint="prefill_idx_qr_rope"):
    ...
    for idx in pl.parallel(0, T * IDX_N_HEADS // QH_QUANT_BLOCK, 1):
        with pl.at(level=pl.Level.CORE_GROUP, name_hint="prefill_idx_qr_hadamard_quant"):
            ...
            for ro in pl.range(0, QH_QUANT_BLOCK, QH_QUANT_ROW_TILE):
                ...
                for h1 in pl.range(0, IDX_HEAD_DIM, HEAD_DIM_TILE):
                    ...
                    qh_q_half = pl.cast(pl.cast(qh_q_i32, target_type=pl.FP32), target_type=pl.FP16, mode="round")
    ...
    for idx in pl.parallel(0, T // WEIGHTS_ROW_TILE, 1):
        with pl.at(level=pl.Level.CORE_GROUP, name_hint="prefill_idx_weights_proj"):
    ...
    with pl.at(level=pl.Level.CORE_GROUP, name_hint="prefill_idx_score"):
        ...
        for cb in pl.range(INDEXER_SCORE_BLOCKS):
            ...
            if max_visible > cache0:
                ...
                kv_q_full_half = pl.cast(pl.cast(kv_q_full_i32, target_type=pl.FP32), target_type=pl.FP16, mode="round")
    ...
    for topk_idx in pl.parallel(0, T // TOPK_TILE, 1):
        with pl.at(level=pl.Level.CORE_GROUP, name_hint="prefill_idx_topk"):
...
def prefill_indexer_test(
    ...
    for tb in pl.parallel(0, T // TOPK_TILE, 1):
        with pl.at(level=pl.Level.CORE_GROUP, name_hint="prefill_idx_topk_copy"):
            t0 = tb * TOPK_TILE
            for ti in pl.range(TOPK_TILE):
                t = t0 + ti
