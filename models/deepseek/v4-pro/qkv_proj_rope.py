...
QR_K_CHUNKS = QR_K_SLICE // QR_K_TILE  # =14; peel 1 + unroll 13
...
KV_K_CHUNKS = KV_K_SLICE // KV_K_TILE  # =14; peel 1 + unroll 13
QPROJ_M_TILE = MATMUL_T_TILE  # qproj token (M) tile; decode pads from 8 real rows to 16
QPROJ_K_CHUNKS = Q_LORA // Q_PROJ_TILE  # =12; peel 1 + unroll 11
...
assert QR_K_CHUNKS == 14 and KV_K_CHUNKS == 14 and QPROJ_K_CHUNKS == 12  # pl.unroll literals below
...
def qkv_proj_rope(
    ...
    with pl.spmd(t_dim // Q_ROPE_T_TILE, name_hint="q_rope_prepare", allow_early_resolve=True) as _qrope_prep_tid:
    ...
    for qbg_idx in pl.spmd(_QR_SPMD, name_hint="qr_proj_matmul", allow_early_resolve=True):
        ...
        for tc in pl.range(t_matmul // QR_M_TILE):
            ...
            for db in pl.unroll(13):  # QR_K_CHUNKS - 1
    ...
    with pl.spmd(_QP_SPMD, name_hint="qproj_matmul", allow_early_resolve=True) as _qproj_tid:
        ...
        for h_inner in pl.range(2):
            ...
            for tc in pl.range(t_matmul // QPROJ_M_TILE):
                ...
                for qb in pl.unroll(11):  # QPROJ_K_CHUNKS - 1
    ...
    with pl.spmd(
        H // Q_ROPE_H_TILE,
        name_hint="qproj_dequant_rms_nope_rope", allow_early_resolve=True,
    ...
    with pl.spmd(_KV_SPMD, name_hint="kv_proj_matmul", deps=[late_dep]) as _kv_tid:
        ...
        for tc in pl.range(t_matmul // KV_M_TILE):
            ...
            for db in pl.unroll(13):  # KV_K_CHUNKS - 1
