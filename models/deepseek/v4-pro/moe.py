...
from config import PRO_KERNEL as M, EP_WORLD_SIZE, MOE_TOKENS, RECV_MAX, MX_BLOCK_K
...
def dispatch(
    ...
    with pl.at(level=pl.Level.CORE_GROUP, name_hint="dispatch_wait", allow_early_resolve=True, deps=[_meta_tid]) as _wait_tid:
        for src in pl.range(N_RANKS):
            if src != my_rank:
                pld.system.wait(
                    signal=data_arrived,
