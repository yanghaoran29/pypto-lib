...
def sparse_attn_swa(
    ...
    with pl.spmd(T, name_hint="qk_pv", allow_early_resolve=True, deps=[gather_tids[0]]) as qk_tid:
    ...
    with pl.spmd(T * (H // H_TILE), name_hint="merge_norm", allow_early_resolve=True, deps=[qk_tid, rope_tid]) as merge_tid:
    ...
    with pl.manual_scope():
        for g in pl.parallel(O_GROUPS):
            ...
            with pl.spmd(PA_NFRAGS, name_hint="proj_a_mm", allow_early_resolve=True, deps=[merge_tid]) as pa_tid:
            ...
            with pl.spmd(PB_DCHUNKS, name_hint="proj_b_mm", allow_early_resolve=True, deps=[pa_tid]) as pb_tid:
    ...
    with pl.spmd(PB_ACT_NREG * PB_ACT_TBLKS, name_hint="proj_b_act", allow_early_resolve=True,
