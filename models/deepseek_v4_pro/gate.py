...
def gate(
    ...
    for quant_idx in pl.spmd((T_PAD // GATE_M_TILE) * (D // QUANT_TILE), name_hint="x_norm_mx_quant"):
        tile_idx = quant_idx // (D // QUANT_TILE)
        chunk_idx = quant_idx % (D // QUANT_TILE)
        t0 = tile_idx * GATE_M_TILE
        k0 = chunk_idx * QUANT_TILE
        xg_chunk = pl.load(xg_buf, [t0, k0], [GATE_M_TILE, QUANT_TILE])
        inv_rms_chunk_row = pl.load(inv_rms_buf, [0, t0], [1, GATE_M_TILE])
        xg_chunk_t = pl.transpose(xg_chunk, axis1=0, axis2=1)
        x_norm_chunk_t = pl.col_expand_mul(xg_chunk_t, inv_rms_chunk_row)
        x_norm_chunk = pl.transpose(x_norm_chunk_t, axis1=0, axis2=1)
        x_norm_buf = pl.store(x_norm_chunk, [t0, k0], x_norm_buf)
        x_quant, scale_quant = pl.quant_mx(x_norm_chunk, group_axis=1)
        x_norm_mx = pl.store(x_quant, [t0, k0], x_norm_mx)
        scale_offset = tile_idx * GATE_M_TILE * MX_SCALE_GROUPS
        scale_offset = scale_offset + chunk_idx * GATE_M_TILE * (QUANT_TILE // MX_GROUP)
