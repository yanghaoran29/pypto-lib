...
def expert_shared(
    ...
    for mt in pl.parallel(T_PAD // SH_M_TILE):
        ...
        for nb_idx in pl.spmd(MOE_INTER // MX_MM_INTER_TILE, name_hint="sh_gate_mx_mm"):
            ...
            xs0 = pl.load(x_local, [ts0, 0], [SH_M_TILE, MX_K_TILE])
            xs_scale0 = pl.load(x_local_scale_mx, [ts0, 0], [SH_M_TILE, MX_K_SCALE_GROUPS])
            w1_k0 = pl.load(shared_w1, [0, n0], [MX_K_TILE, MX_MM_INTER_TILE])
            w1_scale0 = pl.load(shared_w1_scale, [0, n0], [MX_K_SCALE_GROUPS, MX_MM_INTER_TILE])
            gate_acc = pl.matmul_mx(xs0, xs_scale0, w1_k0, w1_scale0)
            for k0 in pl.pipeline(MX_K_TILE, D, MX_K_TILE, stage=2):
                ks = k0 // MX_GROUP
                xs_k = pl.load(x_local, [ts0, k0], [SH_M_TILE, MX_K_TILE])
                xs_scale_k = pl.load(x_local_scale_mx, [ts0, ks], [SH_M_TILE, MX_K_SCALE_GROUPS])
                w1_k = pl.load(shared_w1, [k0, n0], [MX_K_TILE, MX_MM_INTER_TILE])
                w1_scale_k = pl.load(shared_w1_scale, [ks, n0], [MX_K_SCALE_GROUPS, MX_MM_INTER_TILE])
        ...
        for nb_idx in pl.spmd(MOE_INTER // MX_MM_INTER_TILE, name_hint="sh_up_mx_mm"):
            ...
            xs0 = pl.load(x_local, [ts0, 0], [SH_M_TILE, MX_K_TILE])
            xs_scale0 = pl.load(x_local_scale_mx, [ts0, 0], [SH_M_TILE, MX_K_SCALE_GROUPS])
            w3_k0 = pl.load(shared_w3, [0, n0], [MX_K_TILE, MX_MM_INTER_TILE])
            w3_scale0 = pl.load(shared_w3_scale, [0, n0], [MX_K_SCALE_GROUPS, MX_MM_INTER_TILE])
            up_acc = pl.matmul_mx(xs0, xs_scale0, w3_k0, w3_scale0)
            for k0 in pl.pipeline(MX_K_TILE, D, MX_K_TILE, stage=2):
                ks = k0 // MX_GROUP
                xs_k = pl.load(x_local, [ts0, k0], [SH_M_TILE, MX_K_TILE])
                xs_scale_k = pl.load(x_local_scale_mx, [ts0, ks], [SH_M_TILE, MX_K_SCALE_GROUPS])
                w3_k = pl.load(shared_w3, [k0, n0], [MX_K_TILE, MX_MM_INTER_TILE])
                w3_scale_k = pl.load(shared_w3_scale, [ks, n0], [MX_K_SCALE_GROUPS, MX_MM_INTER_TILE])
        ...
        for row_block in pl.spmd(SH_M_TILE // SH_ROW_TILE, name_hint="sh_gate_up_act"):
        ...
        for q_idx in pl.spmd(MOE_INTER // QUANT_TILE, name_hint="sh_h_mx_quant"):
            k0 = q_idx * QUANT_TILE
        ...
        for db_idx in pl.spmd(D // D_OUT_TILE, name_hint="sh_w2_mm"):
            ...
            hs0 = pl.load(h_tile_mx, [0, 0], [SH_M_TILE, MX_K_TILE])
            hs_scale0 = pl.load(h_tile_scale_mx, [0, 0], [SH_M_TILE, MX_K_SCALE_GROUPS])
            sw2_0 = pl.load(shared_w2, [0, d0], [MX_K_TILE, D_OUT_TILE])
            sw2_scale0 = pl.load(shared_w2_scale, [0, d0], [MX_K_SCALE_GROUPS, D_OUT_TILE])
            y_acc = pl.matmul_mx(hs0, hs_scale0, sw2_0, sw2_scale0)
            for k0 in pl.pipeline(MX_K_TILE, MOE_INTER, MX_K_TILE, stage=2):
                ks = k0 // MX_GROUP
                hs_k = pl.load(h_tile_mx, [0, k0], [SH_M_TILE, MX_K_TILE])
                hs_scale_k = pl.load(h_tile_scale_mx, [0, ks], [SH_M_TILE, MX_K_SCALE_GROUPS])
                sw2_k = pl.load(shared_w2, [k0, d0], [MX_K_TILE, D_OUT_TILE])
                sw2_scale_k = pl.load(shared_w2_scale, [ks, d0], [MX_K_SCALE_GROUPS, D_OUT_TILE])
                y_acc = pl.matmul_mx_acc(y_acc, hs_k, hs_scale_k, sw2_k, sw2_scale_k)
