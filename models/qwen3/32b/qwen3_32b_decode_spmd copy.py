...
def build_qwen3_decode_program():
    ...
    mlp_spmd_outer_blocks = mlp_out_blocks // mlp_spmd_inner
    mlp_group_chunk = mlp_spmd_inner * MLP_OUT_CHUNK
    out_proj_blocks_s3 = hidden // out_proj_chunk
    ...
    assert q_out_blocks % out_proj_spmd_inner == 0
    assert hidden % out_proj_chunk == 0
    ...
    class Qwen3Decode:
        ...
        def qwen3_decode(
            ...
            for b0 in pl.parallel(0, batch, BATCH_TILE):
                ...
                for ob_base in pl.parallel(0, out_proj_blocks_s3, out_proj_spmd_inner):
                    for ob in pl.spmd(out_proj_spmd_inner, level=pl.Level.CORE_GROUP, name_hint="out_proj_residual"):
                        o0 = (ob_base + ob) * out_proj_chunk
                        hidden_chunk = pl.slice(hidden_states, [BATCH_TILE, out_proj_chunk], [b0, o0])
                        o_acc = pl.create_tensor([BATCH_TILE, out_proj_chunk], dtype=pl.FP32)
                        for kb in pl.pipeline(0, hidden_blocks, stage=2):
                            k0 = kb * K_CHUNK
                            a_chunk = pl.slice(attn_out, [BATCH_TILE, K_CHUNK], [b0, k0])
                            w_chunk = pl.slice(wo, [K_CHUNK, out_proj_chunk], [k0, o0])
                            if k0 == 0:
                ...
                for ob_base in pl.parallel(0, mlp_out_blocks, mlp_spmd_inner):
                    ...
                    # Stage 4: gate projection (non-SPMD).
                    for ob in pl.range(mlp_spmd_inner):
                    ...
                    # Stage 5: up projection (non-SPMD).
                    for ob in pl.range(mlp_spmd_inner):
                    ...
                    # Stage 6: SiLU + gate/up fuse (non-SPMD).
                    for ob in pl.range(mlp_spmd_inner):
