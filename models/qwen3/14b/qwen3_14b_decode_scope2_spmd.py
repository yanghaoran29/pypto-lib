...
# -----------------------------------------------------------------------------------------------------------
"""Qwen3-14B decode Scope 2 (SPMD variant) — q/k norm + RoPE + paged KV update + decode attention.

Ports the SPMD layout from ``models/qwen3/32b/qwen3_32b_decode.py``:

- Outer ``pl.parallel(batch)`` over batch.
- Stage 1: K RoPE + cache update + V cache + Q RoPE + pad inside one
  ``pl.at(CORE_GROUP)`` block iterating over KV heads (``pl.range``).
  q/k RMS-norm is folded in here so we can drop the global pre-pass.
- Stages 2-5: paired SPMD over ``total_q_groups // 2`` (gi0/gi1 per worker).

PagedAttention contract is preserved: ``block_table`` indexes physical KV
blocks; ``slot_mapping`` is the write slot for the current decode token.
...
def build_qwen3_scope2_program(
    ...
    class Qwen3Scope2:
        ...
        def qwen3_scope2(
            ...
            # Per-head q/k RMS-norm at full batch width (batch >= 16 keeps
            # 32B-aligned tiles required by the a2a3 vec ISA).
            with pl.at(level=pl.Level.CORE_GROUP, name_hint="qk_rmsnorm"):
                for h in pl.range(num_heads):
                    q0 = h * head_dim
                    q_chunk = pl.slice(q_proj, [batch, head_dim], [0, q0])
                    ...
                    q_proj_norm = pl.assemble(q_proj_norm, q_chunk_norm, [0, q0])
                ...
                for h in pl.range(num_kv_heads):
                    ...
                    k_proj_norm = pl.assemble(k_proj_norm, k_chunk_norm, [0, k0])
            ...
            for b in pl.parallel(batch):
                ...
                cos_lo = pl.slice(rope_cos, [1, half_dim], [pos, 0])
                cos_hi = pl.slice(rope_cos, [1, half_dim], [pos, half_dim])
                sin_lo = pl.slice(rope_sin, [1, half_dim], [pos, 0])
                sin_hi = pl.slice(rope_sin, [1, half_dim], [pos, half_dim])

                # Stage 1: K RoPE + KV cache write + Q RoPE + pad.
                with pl.at(level=pl.Level.CORE_GROUP, name_hint="rope_kv_cache"):
                    for ki in pl.range(num_kv_heads):
                        kv_col = ki * head_dim

                        # K RoPE on the already-normalized k_proj_norm row.
                        k_chunk_row = pl.slice(k_proj_norm, [1, head_dim], [b, kv_col])
                        k_lo = pl.slice(k_chunk_row, [1, half_dim], [0, 0])
                        k_hi = pl.slice(k_chunk_row, [1, half_dim], [0, half_dim])
                        ...
                        # V cache write.
                        v_row_bf16 = pl.cast(pl.slice(v_proj, [1, head_dim], [b, kv_col]), target_type=pl.BF16)
                        v_cache = pl.assemble(v_cache, v_row_bf16, [cache_row, 0])

                        # Q RoPE + pad (one query head per iter, same layout as scope2.py).
                        q_base = ki * q_per_kv
                ...
                for gi in pl.spmd(total_q_groups // 2, name_hint="qk_matmul"):
                    ...
                    q_padded0 = pl.slice(all_q_padded, [Q_HEAD_PAD, head_dim], [q_pad_row0_0, 0])
                    q_padded1 = pl.slice(all_q_padded, [Q_HEAD_PAD, head_dim], [q_pad_row0_1, 0])
                    for sb in pl.range(ctx_blocks):
                        ...
                        cache_row0_0 = (pbid * num_kv_heads + kvh0) * BLOCK_SIZE
                        ...
                        cache_row0_1 = (pbid * num_kv_heads + kvh1) * BLOCK_SIZE
                ...
                # Stage 3: softmax (paired SPMD).
                all_exp_padded = pl.create_tensor(
                    [total_q_groups * max_ctx_blocks * Q_HEAD_PAD, BLOCK_SIZE],
                    dtype=pl.BF16,
                )
                # mi/li intermediates use Q_HEAD_PAD-wide tiles so the [N, 1]
                # FP32 ColMajor reduction outputs stay 32B-aligned (a2a3 vec ISA).
                ...
                # Stage 5: online-softmax accumulation + final normalisation (paired SPMD).
                for gi in pl.spmd(total_q_groups // 2, name_hint="online_softmax"):
                    ...
                    # Operate on Q_HEAD_PAD-wide tiles to keep [N, 1] FP32
                    # ColMajor reductions 32B-aligned; valid heads are the
                    # first Q_HEAD_BATCH rows, taken when writing attn_row.
                    oi_0 = pl.slice(all_oi_tmp, [Q_HEAD_PAD, head_dim], [gi0 * max_ctx_blocks * Q_HEAD_PAD, 0])
                    mi_0 = pl.slice(all_cur_mi, [Q_HEAD_PAD, 1], [gi0 * max_ctx_blocks * Q_HEAD_PAD, 0])
                    li_0 = pl.slice(all_cur_li, [Q_HEAD_PAD, 1], [gi0 * max_ctx_blocks * Q_HEAD_PAD, 0])
                    oi_1 = pl.slice(all_oi_tmp, [Q_HEAD_PAD, head_dim], [gi1 * max_ctx_blocks * Q_HEAD_PAD, 0])
                    mi_1 = pl.slice(all_cur_mi, [Q_HEAD_PAD, 1], [gi1 * max_ctx_blocks * Q_HEAD_PAD, 0])
                    li_1 = pl.slice(all_cur_li, [Q_HEAD_PAD, 1], [gi1 * max_ctx_blocks * Q_HEAD_PAD, 0])
                    for sb in pl.range(1, ctx_blocks):
                        oi_tmp_valid_0 = pl.slice(
                            all_oi_tmp,
                            [Q_HEAD_PAD, head_dim],
                            [gi0 * max_ctx_blocks * Q_HEAD_PAD + sb * Q_HEAD_PAD, 0],
                        )
                        ...
                        oi_0 = pl.add(
                            pl.row_expand_mul(oi_0, alpha_0),
                            pl.row_expand_mul(oi_tmp_valid_0, beta_0),
                        )
                        mi_0 = mi_new_0

                        oi_tmp_valid_1 = pl.slice(
                            all_oi_tmp,
                            [Q_HEAD_PAD, head_dim],
                            [gi1 * max_ctx_blocks * Q_HEAD_PAD + sb * Q_HEAD_PAD, 0],
                        )
                        ...
                        oi_1 = pl.add(
                            pl.row_expand_mul(oi_1, alpha_1),
                            pl.row_expand_mul(oi_tmp_valid_1, beta_1),
                        )
                        mi_1 = mi_new_1

                    ctx_padded_0 = pl.row_expand_div(oi_0, li_0)
                    ...
                    ctx_padded_1 = pl.row_expand_div(oi_1, li_1)
