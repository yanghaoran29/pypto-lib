        ...
        def qwen3_decode(
            ...
            for b in pl.parallel(user_batch):
                ...
                for gi in pl.spmd(
                    ...
                    for sb in pl.range(ctx_blocks):
                        ...
                        # gi0: qk matmul (AIC) → softmax (AIV) via outer-scope GM
                        ...
                        # gi1: qk matmul (AIC) → softmax (AIV) via outer-scope GM
            ...
            for b0 in pl.parallel(0, batch_padded, BATCH_TILE):
                ...
                for ob_pair_base in pl.spmd(
                    ...
                    # out_oi1_idx = ob_pair + 1
                    # o0 = out_oi1_idx * OUT_PROJ_N_CHUNK
                    # hidden_chunk = pl.slice(
                    #     hidden_states,
                    #     [BATCH_TILE, OUT_PROJ_N_CHUNK],
                    #     [b0, o0],
                ...
                for ob_base in pl.parallel(0, mlp_out_blocks, MLP_SPMD_INNER):
                    ...
                    for ob in pl.spmd(MLP_SPMD_INNER, name_hint="gate_proj_spmd"):
                        mlp_o0 = (ob_base + ob) * MLP_OUT_CHUNK
                        g0 = ob * MLP_OUT_CHUNK
                        ...
                        wg_1 = pl.slice(w_gate, [K_CHUNK, MLP_OUT_CHUNK], [K_CHUNK, mlp_o0])
                    ...
                    for ob in pl.spmd(MLP_SPMD_INNER, name_hint="up_proj_spmd"):
                        ...
                        wu_1 = pl.slice(w_up, [K_CHUNK, MLP_OUT_CHUNK], [K_CHUNK, mlp_o0])
                ...
                for di in pl.spmd(down_out_blocks, name_hint="down_proj_spmd"):
                    ...
                    for ob in pl.pipeline(0, down_mlp_blocks, stage=2):
                        down_o0 = ob * DOWN_MLP_CHUNK
                        down_mlp_chunk_bf16 = mlp_tile[:, down_o0 : down_o0 + DOWN_MLP_CHUNK]
                        w_down_chunk = w_down[down_o0 : down_o0 + DOWN_MLP_CHUNK, d0 : d0 + DOWN_OUT_CHUNK]
                        if down_o0 == 0:
                ...
                for db in pl.parallel(0, down_out_blocks):
                    with pl.at(level=pl.Level.CORE_GROUP, optimizations=[pl.split(pl.SplitMode.NONE)], name_hint="down_proj_residual"):
                        dr0 = db * DOWN_OUT_CHUNK
                        resid_chunk_fp32 = resid1_tile[:, dr0 : dr0 + DOWN_OUT_CHUNK]
                        down_acc_r = pl.create_tensor([BATCH_TILE, DOWN_OUT_CHUNK], dtype=pl.FP32)
                        for ob in pl.pipeline(0, down_mlp_blocks, stage=2):
                            mk0 = ob * DOWN_MLP_CHUNK
                            down_mlp_chunk_bf16 = mlp_tile[:, mk0 : mk0 + DOWN_MLP_CHUNK]
                            w_down_chunk = w_down[mk0 : mk0 + DOWN_MLP_CHUNK, dr0 : dr0 + DOWN_OUT_CHUNK]
                            if mk0 == 0:
                                down_acc_r = pl.matmul(down_mlp_chunk_bf16, w_down_chunk, out_dtype=pl.FP32)
                            else:
                                down_acc_r = pl.matmul_acc(down_acc_r, down_mlp_chunk_bf16, w_down_chunk)
                        out_chunk = pl.add(down_acc_r, resid_chunk_fp32)
                        ...
                        out = pl.assemble(out, out_chunk_trimmed, [b0, dr0])
