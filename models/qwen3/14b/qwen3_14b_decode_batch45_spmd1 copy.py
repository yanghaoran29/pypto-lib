...
def build_qwen3_decode_program(
    ...
    class Qwen3Decode:
        ...
        def qwen3_decode(
            ...
            normed_full = pl.create_tensor([batch_padded, hidden], dtype=pl.BF16)

            for tile in pl.spmd(batch_tile_count, name_hint="rmsnorm", sync_start=True):
                b0 = tile * BATCH_TILE
            ...
            for b0 in pl.parallel(0, batch_padded, BATCH_TILE):
                normed_tile = pl.slice(normed_full, [BATCH_TILE, hidden], [b0, 0])
                ...
                for kvi in pl.spmd(kv_out_blocks, name_hint="k_proj"):
                ...
                for vvi in pl.spmd(kv_out_blocks, name_hint="v_proj"):
            ...
            for b in pl.parallel(user_batch):
                ...
                for gi in pl.spmd(total_q_groups, name_hint="qk_matmul"):
                    kvh = gi // q_groups
                    ...
                    for sb in pl.range(ctx_blocks):
                        ...
                        all_raw_scores = pl.assemble(
                            all_raw_scores,
                            raw_scores,
                            [gi * max_ctx_blocks * Q_HEAD_PAD + sb * Q_HEAD_PAD, 0],
                        )
                ...
                for gi in pl.spmd(total_q_groups, name_hint="softmax"):
                    for sb in pl.range(ctx_blocks):
                        s0 = sb * BLOCK_SIZE
                ...
                for gi in pl.spmd(total_q_groups, name_hint="sv_matmul"):
                    kvh = gi // q_groups
                    for sb in pl.range(ctx_blocks):
                        block_table_idx = block_table_base + sb
                        pbid = pl.cast(pl.tensor.read(block_table, [block_table_idx]), pl.INDEX)
                        ...
                        v_tile = pl.slice(v_cache, [BLOCK_SIZE, head_dim], [cache_row, 0])
                ...
                for gi in pl.spmd(total_q_groups, name_hint="online_softmax"):
                    kvh = gi // q_groups
                    qg = gi - kvh * q_groups
                    q_base = kvh * q_per_kv + qg * Q_HEAD_BATCH

                    oi = pl.slice(all_oi_tmp, [Q_HEAD_PAD, head_dim], [gi * max_ctx_blocks * Q_HEAD_PAD, 0])
                    mi = pl.slice(all_cur_mi, [Q_HEAD_PAD, 1], [gi * max_ctx_blocks * Q_HEAD_PAD, 0])
                    li = pl.slice(all_cur_li, [Q_HEAD_PAD, 1], [gi * max_ctx_blocks * Q_HEAD_PAD, 0])
                    ...
                    ctx_padded = pl.row_expand_div(oi, li)
                    ctx_valid = pl.slice(ctx_padded, [Q_HEAD_BATCH, head_dim], [0, 0])
            ...
            for b0 in pl.parallel(0, batch_padded, BATCH_TILE):
                ...
                #     out_proj_n_blocks // 2,
                #     optimizations=[pl.split(pl.SplitMode.NONE)],
                #     name_hint="out_proj_residual_1", 
                #     sync_start=True
                # ):
                #     ob_pair = ob_pair_base * 2
                ...
                for di in pl.parallel(down_out_blocks):
