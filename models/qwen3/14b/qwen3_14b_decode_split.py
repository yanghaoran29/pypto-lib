...
# -----------------------------------------------------------------------------------------------------------
"""Qwen3-14B single-layer decode forward (K/V projection split variant).

``q_proj`` uses a single ``pl.at`` per output tile (same as ``qwen3_14b_decode.py``).
``k_proj`` / ``v_proj`` each use two ``pl.at`` tasks per tile by halving the N
dimension (256 -> 2x128). Math matches the single-task layout in
``qwen3_14b_decode.py``.

**Reference build (split profiling / 编排真值)**:
``build_output/Qwen3Decode_20260514_115206/`` — use this tree when comparing
``kernel_config.py`` (note ``func_id`` shift: ``k_proj_0``/``v_proj_0`` are
``func_id`` 4/5) and ``dfx_outputs/l2_perf_records.json``. Example run inferred
``user_batch=90``, ``batch_padded=96`` (six ``BATCH_TILE`` rows), DFX total
3588 L2 task records.
...
Q_OUT_CHUNK = 256
KV_OUT_CHUNK = 256
# Half output width per K/V projection sub-task (two tasks per original 256-wide tile).
KV_OUT_HALF = KV_OUT_CHUNK // 2
BATCH_TILE = 16
        ...
        def qwen3_decode(
            ...
            for b0 in pl.parallel(0, batch_padded, BATCH_TILE):
                ...
                for kv0 in pl.parallel(0, kv_hidden, KV_OUT_CHUNK):
                    ...
                    with pl.at(level=pl.Level.CORE_GROUP, name_hint="k_proj"):
                        k_acc = pl.create_tensor([BATCH_TILE, KV_OUT_HALF], dtype=pl.FP32)
                        for kb in pl.pipeline(0, kv_proj_k_blocks, stage=2):
                            k0 = kb * KV_PROJ_K_CHUNK
                            k_tile_a_i = normed_tile[:, k0 : k0 + KV_PROJ_K_CHUNK]
                            k_tile_w_i = wk[k0 : k0 + KV_PROJ_K_CHUNK, kv_col_a : kv_col_a + KV_OUT_HALF]
                            if k0 == 0:
                                k_acc = pl.matmul(k_tile_a_i, k_tile_w_i, out_dtype=pl.FP32)
                            else:
                                k_acc = pl.matmul_acc(k_acc, k_tile_a_i, k_tile_w_i)
                        k_proj = pl.assemble(k_proj, k_acc, [b0, kv_col_a])

                    with pl.at(level=pl.Level.CORE_GROUP, name_hint="v_proj"):
                        v_acc = pl.create_tensor([BATCH_TILE, KV_OUT_HALF], dtype=pl.FP32)
                        for kb in pl.pipeline(0, kv_proj_k_blocks, stage=2):
                            k0 = kb * KV_PROJ_K_CHUNK
                            v_tile_a_i = normed_tile[:, k0 : k0 + KV_PROJ_K_CHUNK]
                            v_tile_w_i = wv[k0 : k0 + KV_PROJ_K_CHUNK, kv_col_a : kv_col_a + KV_OUT_HALF]
                            if k0 == 0:
                                v_acc = pl.matmul(v_tile_a_i, v_tile_w_i, out_dtype=pl.FP32)
                            else:
                                v_acc = pl.matmul_acc(v_acc, v_tile_a_i, v_tile_w_i)
                        v_proj = pl.assemble(v_proj, v_acc, [b0, kv_col_a])

                    kv_col_b = kv0 + KV_OUT_HALF
                    with pl.at(level=pl.Level.CORE_GROUP, name_hint="k_proj"):
                        k_acc = pl.create_tensor([BATCH_TILE, KV_OUT_HALF], dtype=pl.FP32)
                        for kb in pl.pipeline(0, kv_proj_k_blocks, stage=2):
                            k0 = kb * KV_PROJ_K_CHUNK
                            k_tile_a_i = normed_tile[:, k0 : k0 + KV_PROJ_K_CHUNK]
                            k_tile_w_i = wk[k0 : k0 + KV_PROJ_K_CHUNK, kv_col_b : kv_col_b + KV_OUT_HALF]
                            if k0 == 0:
                                k_acc = pl.matmul(k_tile_a_i, k_tile_w_i, out_dtype=pl.FP32)
                            else:
                                k_acc = pl.matmul_acc(k_acc, k_tile_a_i, k_tile_w_i)
                        k_proj = pl.assemble(k_proj, k_acc, [b0, kv_col_b])
                    ...
                    with pl.at(level=pl.Level.CORE_GROUP, name_hint="v_proj"):
                        ...
                        for kb in pl.pipeline(0, kv_proj_k_blocks, stage=2):
                            k0 = kb * KV_PROJ_K_CHUNK
                            v_tile_a_i = normed_tile[:, k0 : k0 + KV_PROJ_K_CHUNK]
                            v_tile_w_i = wv[k0 : k0 + KV_PROJ_K_CHUNK, kv_col_b : kv_col_b + KV_OUT_HALF]
                            if k0 == 0:
                                v_acc = pl.matmul(v_tile_a_i, v_tile_w_i, out_dtype=pl.FP32)
                            else:
                                v_acc = pl.matmul_acc(v_acc, v_tile_a_i, v_tile_w_i)
                        v_proj = pl.assemble(v_proj, v_acc, [b0, kv_col_b])
            ...
            for b in pl.parallel(user_batch):
                ...
                for gi in pl.parallel(0, total_q_groups, 2):
                    ...
                    q_padded0 = all_q_padded[q_padded_row0 : q_padded_row0 + Q_HEAD_PAD, :]
            ...
            #
            # Output projection + residual: same two-task pattern as down_proj /
            # down_proj_residual — GM scratch per N-chunk, then separate CORE_GROUP
            # regions so lowering outlines AIC (matmul) vs AIV (slice/cast/add).
            for b0 in pl.parallel(0, batch_padded, BATCH_TILE):
                cur_valid = pl.min(BATCH_TILE, user_batch - b0)
