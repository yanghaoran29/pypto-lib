...
# -----------------------------------------------------------------------------------------------------------
"""Qwen3-14B single-layer decode.

Scope 1: RMSNorm + Q/K/V projection all use ``pl.spmd`` over output tiles.
Q, K, and V each get a separate ``pl.spmd`` loop so each body holds only one
matmul pipeline (same 256×256 tiling as ``decode``; a single fused K+V
``spmd`` would exceed the a2a3 mat buffer limit).

Scope 3 follows ``qwen3_14b_decode_scope3_spmd.py``: paired ``pl.parallel`` on
out-proj tiles, inner ``pl.spmd`` on MLP gate/up/SiLU groups; down projection
uses two ``pl.spmd`` passes over output columns (matmul then residual), like
separate K/V ``spmd`` loops.

Scope 2 uses ``pl.parallel(0, total_q_groups, 2)`` + dual buffers (same as
``qwen3_14b_decode.py``); attention ``pl.spmd`` under ``pl.parallel(user_batch)``
is not used here.
"""
...
# Scope 1 tiling constants (same names/values as ``qwen3_14b_decode.py``).
...
def build_qwen3_decode_spmd_program(
    batch: int = BATCH,
    max_seq: int = MAX_SEQ,
    ...
    class Qwen3DecodeSpmd:
        ...
        def qwen3_decode(
            ...
            for b0 in pl.parallel(0, batch_padded, BATCH_TILE):
                ...
                for qi in pl.spmd(q_out_blocks, name_hint="q_proj_spmd"):
                    q0 = qi * Q_OUT_CHUNK
                ...
                for kvi in pl.spmd(kv_out_blocks, name_hint="k_proj_spmd"):
                ...
                for vvi in pl.spmd(kv_out_blocks, name_hint="v_proj_spmd"):
                    kv0 = vvi * KV_OUT_CHUNK
            ...
            # Attention stages use paired SPMD.
            ...
            for b in pl.parallel(user_batch):
                ...
                with pl.at(level=pl.Level.CORE_GROUP, name_hint="rope_kv_cache"):
                    for ki in pl.range(num_kv_heads):
                        ...
                        v_cache = pl.assemble(
                            v_cache,
                            pl.cast(
                                v_proj[b : b + 1, kv_col : kv_col + head_dim],
                                target_type=pl.BF16,
                            ),
                            [cache_row, 0],
                        )
                        q_base = ki * q_per_kv
                        q_block = pl.reshape(
                            q_proj_norm[b : b + 1, q_base * head_dim : (q_base + Q_HEAD_BATCH) * head_dim],
                ...
                for gi in pl.spmd(total_q_groups // 2, name_hint="qk_matmul"):
                    ...
                    for sb in pl.range(ctx_blocks):
                        ...
                        k_tile0 = pl.slice(k_cache, [BLOCK_SIZE, head_dim], [cache_row0, 0])
                        ...
                        k_tile1 = pl.slice(k_cache, [BLOCK_SIZE, head_dim], [cache_row1, 0])
                ...
                for gi in pl.spmd(total_q_groups // 2, name_hint="sv_matmul"):
                    ...
                    for sb in pl.range(ctx_blocks):
                        ...
                        v_tile0 = pl.slice(v_cache, [BLOCK_SIZE, head_dim], [cache_row0, 0])
                        ...
                        v_tile1 = pl.slice(v_cache, [BLOCK_SIZE, head_dim], [cache_row1, 0])
                ...
                for gi in pl.spmd(total_q_groups // 2, name_hint="online_softmax"):
                    ...
                    oi0 = pl.slice(all_oi_tmp, [Q_HEAD_PAD, head_dim], [gi0 * max_ctx_blocks * Q_HEAD_PAD, 0])
                    mi0 = pl.slice(all_cur_mi, [Q_HEAD_PAD, 1], [gi0 * max_ctx_blocks * Q_HEAD_PAD, 0])
                    li0 = pl.slice(all_cur_li, [Q_HEAD_PAD, 1], [gi0 * max_ctx_blocks * Q_HEAD_PAD, 0])
                    oi1 = pl.slice(all_oi_tmp, [Q_HEAD_PAD, head_dim], [gi1 * max_ctx_blocks * Q_HEAD_PAD, 0])
                    mi1 = pl.slice(all_cur_mi, [Q_HEAD_PAD, 1], [gi1 * max_ctx_blocks * Q_HEAD_PAD, 0])
                    li1 = pl.slice(all_cur_li, [Q_HEAD_PAD, 1], [gi1 * max_ctx_blocks * Q_HEAD_PAD, 0])
                    for sb in pl.range(1, ctx_blocks):
                        oi_tmp_valid0 = pl.slice(
                            all_oi_tmp,
                            [Q_HEAD_PAD, head_dim],
                            [gi0 * max_ctx_blocks * Q_HEAD_PAD + sb * Q_HEAD_PAD, 0],
                        )
                        ...
                        oi0 = pl.add(
                            pl.row_expand_mul(oi0, alpha0),
                            pl.row_expand_mul(oi_tmp_valid0, beta0),
                        )
                        mi0 = mi_new0

                        oi_tmp_valid1 = pl.slice(
                            all_oi_tmp,
                            [Q_HEAD_PAD, head_dim],
                            [gi1 * max_ctx_blocks * Q_HEAD_PAD + sb * Q_HEAD_PAD, 0],
                        )
                        ...
                        oi1 = pl.add(
                            pl.row_expand_mul(oi1, alpha1),
                            pl.row_expand_mul(oi_tmp_valid1, beta1),
                        )
                        mi1 = mi_new1
            ...
            # Scope 3: layout from ``qwen3_14b_decode_scope3_spmd.py`` (paired
            ...
            for b0 in pl.parallel(0, batch_padded, BATCH_TILE):
                ...
                for ob_pair0 in pl.spmd( out_proj_n_blocks // 2, optimizations=[pl.auto_chunk, pl.split(pl.SplitMode.UP_DOWN)], name_hint="out_proj_residual"):
                        ob_pair = ob_pair0 * 2
                ...
                down_fp32_tile = pl.create_tensor([BATCH_TILE, hidden], dtype=pl.FP32)
                for di in pl.spmd(down_out_blocks, name_hint="down_proj_spmd"):
                    d0 = di * DOWN_OUT_CHUNK
                ...
                for di in pl.spmd(down_out_blocks, name_hint="down_proj_residual_spmd"):
                    d0 = di * DOWN_OUT_CHUNK
                    down_chunk_fp32 = pl.slice(down_fp32_tile, [BATCH_TILE, DOWN_OUT_CHUNK], [0, d0])
...
if __name__ == "__main__":
    ...
    result = run(
        program=build_qwen3_decode_spmd_program(batch=args.batch),
