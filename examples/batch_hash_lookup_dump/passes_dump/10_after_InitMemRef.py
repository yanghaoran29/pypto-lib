...
class BatchHashLookup:
    ...
    @pl.function(type=pl.FunctionType.InCore)
    def batch_hash_lookup_incore_1(self, hash_pool_0: pl.Tensor[[64, 128, 32], pl.INT32, pl.MemRef(pl.MemorySpace.DDR, -1, 1048576, 0)], probe_0: pl.Scalar[pl.INDEX], search_key_0: pl.Tensor[[1024, 64, 32], pl.INT32, pl.MemRef(pl.MemorySpace.DDR, -1, 8388608, 1)], ti_0: pl.Scalar[pl.INDEX], ti_iter_1: pl.Scalar[pl.INDEX], value_ptr_out_2: pl.Tensor[[1024, 64, 32], pl.INT32, pl.MemRef(pl.MemorySpace.DDR, -1, 8388608, 2)], value_ptr_out_iter_6: pl.Tensor[[1024, 64, 32], pl.INT32, pl.MemRef(pl.MemorySpace.DDR, -1, 8388608, 3)]) -> tuple[pl.Scalar[pl.INDEX], pl.Scalar[pl.INDEX], pl.Tensor[[1024, 64, 32], pl.INT32]]:
        for b_3, (ti_iter_3, value_ptr_out_iter_8) in pl.parallel(0, 1024, 32, init_values=(ti_iter_1, value_ptr_out_iter_6)):
            for ti_5, (value_ptr_out_iter_10,) in pl.parallel(0, 64, 32, init_values=(value_ptr_out_iter_8,)):
                ...
                key_match_0: pl.Tensor[[1, 32], pl.INDEX, pl.MemRef(pl.MemorySpace.DDR, -1, 256, 19)] = pl.tensor.cmp(cand_key_2, keys_tile_0, cmp_type=0)
                hit_mask_0: pl.Tensor[[1, 32], pl.INDEX, pl.MemRef(pl.MemorySpace.DDR, -1, 256, 20)] = pl.tensor.and(active_mask_0, key_match_0)
                result_next_0: pl.Tensor[[1, 32], pl.INDEX, pl.MemRef(pl.MemorySpace.DDR, -1, 256, 21)] = pl.tensor.sel(hit_mask_0, cand_val_2, result_prev_0)
                value_ptr_out_12: pl.Tensor[[1024, 64, 32], pl.INT32, pl.MemRef(pl.MemorySpace.DDR, -1, 8388608, 22)] = pl.tensor.assemble(value_ptr_out_iter_10, result_next_0, [b_3, ti_5, 0])
                value_ptr_out_11: pl.Tensor[[1024, 64, 32], pl.INT32, pl.MemRef(pl.MemorySpace.DDR, -1, 8388608, 23)] = pl.yield_(value_ptr_out_12)
            ti_4, value_ptr_out_9 = pl.yield_(ti_5, value_ptr_out_11)
        return b_3, ti_4, value_ptr_out_9
    ...
    def batch_hash_lookup(self, search_key_0: pl.Tensor[[1024, 64, 32], pl.INT32, pl.MemRef(pl.MemorySpace.DDR, -1, 8388608, 0)], hash_table_size_0: pl.Tensor[[64, 32], pl.INT32, pl.MemRef(pl.MemorySpace.DDR, -1, 8192, 1)], hash_base_ptr_0: pl.Tensor[[64, 32], pl.INT32, pl.MemRef(pl.MemorySpace.DDR, -1, 8192, 2)], hash_pool_0: pl.Tensor[[64, 128, 32], pl.INT32, pl.MemRef(pl.MemorySpace.DDR, -1, 1048576, 3)], value_ptr_out_0: pl.Tensor[[1024, 64, 32], pl.INT32, pl.MemRef(pl.MemorySpace.DDR, -1, 8388608, 4)]) -> pl.Tensor[[1024, 64, 32], pl.INT32]:
        ...
        for probe_0, (b_iter_1, ti_iter_1, value_ptr_out_iter_6) in pl.range(0, 8, 1, init_values=(b_0, ti_0, value_ptr_out_2)):
            ret: pl.Tuple([pl.Scalar[pl.INDEX], pl.Scalar[pl.INDEX], pl.Tensor[[1024, 64, 32], pl.INT32]]) = self.batch_hash_lookup_incore_1(hash_pool_0, probe_0, search_key_0, ti_0, ti_iter_1, value_ptr_out_2, value_ptr_out_iter_6)
            b_3: pl.Scalar[pl.INDEX] = ret[0]
            ti_4: pl.Scalar[pl.INDEX] = ret[1]
            value_ptr_out_9: pl.Tensor[[1024, 64, 32], pl.INT32, pl.MemRef(pl.MemorySpace.DDR, -1, 8388608, 7)] = ret[2]
